...
package frc.ExternalLib.SpectrumLib.gamepads.mapping;
...
public class SplineCurve extends Curve {
     ...
     * @param deadzone   value for the width of the deadband in the center of the
     *                   curve
     */
    ...
    public SplineCurve() {
        ...
        points = new double[][] { { -1.0, -1.0 }, { 0.0, 0.0 }, { 1.0, 1.0 } }; // have to do this to avoid exception
                                                                                // handling
        setOffset(0.0);
        setScalar(1.0);
        setDeadzone(0.0);
                ...
                switch (splineType) {
                    case SMOOTH:
                        if (input <= points[1][0]) {
                            ...
                            val = calculateSplineSegmentVal(input, SegmentType.RIGHT_PARABOLIC,
    ...
    private double calculateCubicSplineVal(double input, double[] pointA, double[] pointB) {
        ...
        return ((-2 * deltaY) / (deltaX * deltaX * deltaX)) * (transInput * transInput * transInput)
                + ((3 * deltaY) / (deltaX * deltaX)) * (transInput * transInput) + pointA[1];
    }
